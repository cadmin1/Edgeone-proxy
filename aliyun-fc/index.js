...
/**
 * MiniMax proxy ported to Alibaba Cloud Function Compute (函数计算 FC).
 *
 * Trigger: HTTP 触发器 (事件函数, 请求处理程序=处理HTTP请求), Node.js 18+ 运行时.
...
exports.handler = async (event, context) => {
  ...
  // 路径兼容 FC 2.0 (event.path) 与 FC 3.0 (rawPath / requestContext.http.path)
  const rawPath = event.path || http.path || event.rawPath || '/';
  let path = rawPath;
  try {
  ...
  try {
    ...
    responseHeaders['X-Proxied-By'] = 'Alibaba-FC-Proxy';
