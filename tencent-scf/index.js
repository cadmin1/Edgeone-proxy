...
/**
 * MiniMax proxy ported to Tencent Cloud SCF (云函数).
 *
 * Trigger: 函数 URL（Function URL，API 网关已停服）, Node.js 18+ 运行时.
 ...
 * 注：函数 URL 事件查询参数在 event.queryString（兼容 queryStringParameters/queryParameters），
 *     且不会强制 Content-Disposition: attachment，浏览器可直接渲染页面。
 */
...
exports.main_handler = async (event, context) => {
  // FC 事件函数「处理函数请求」模式下，event 可能是 JSON 字符串、JSON 字符串的 Buffer
  // （字节数组）、或已解析对象，统一归一化为对象
  event = normalizeEvent(event);
  if (!event || typeof event !== 'object' || Array.isArray(event)) {
    return buildResponse(
      400,
      'Invalid event | typeof=' + typeof event + ' | isView=' + (event && ArrayBuffer.isView(event)) + ' | raw=' + String(event).slice(0, 300),
      textHeaders()
    );
  }
  const reqContext = event.requestContext || {};
  const http = reqContext.http || {};
  const method = event.httpMethod || http.method || 'GET';
  const clientHeaders = normalizeHeaders(event.headers || {});
  // 查询参数兼容各平台字段名：阿里FC queryParameters / 腾讯SCF queryStringParameters|queryString / 华为 queryStringParameters
  const query = event.queryParameters || event.queryStringParameters || event.queryString || {};
  // 路径兼容：SCF event.path / FC 2.0 event.path / FC 3.0 rawPath / requestContext 内路径
  const rawPath = event.path || http.path || (reqContext.path) || event.rawPath || '/';
  let path = rawPath;
  try {
  ...
  if (!targetUrl) {
    const debugInfo = {
      ...
      path: event.path,
      requestContextHttp: event.requestContext && event.requestContext.http
    };
    return buildResponse(
      400,
      'Missing target URL parameter | event=' + JSON.stringify(debugInfo),
      textHeaders()
    );
  }
...
function normalizeEvent(e) {
  ...
  if (e && typeof e === 'object' && ArrayBuffer.isView(e)) {
    let raw;
    try {
      raw = Buffer.from(e.buffer, e.byteOffset, e.byteLength).toString('utf8');
    } catch {
      raw = String(e);
    }
    try {
